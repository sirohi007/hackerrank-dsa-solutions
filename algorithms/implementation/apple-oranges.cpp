...
void countApplesAndOranges(int homeStart, int homeEnd, int appleTreeAt, int orageTreeAt, vector<int> apples, vector<int> oranges,int m, int n) {
    int p=0;
    int q=0;
    for(int i=0;i<m;i++)
    {
        int appleFallAt= apples[i]+appleTreeAt;
        if(appleFallAt>=homeStart && appleFallAt<=homeEnd)
        {
            p+=1;
        }
    }
    for(int i=0;i<n;i++)
    {
        int orageFallAt= oranges[i]+orageTreeAt;
        if(orageFallAt>=homeStart && orageFallAt<=homeEnd)
        {
            q+=1;
        }
    }
